...
class CTDAnalyzerApp2Sensor:
    def __init__(self, root):
        self.root = root
        self.root.title("CTD Sensor Depth Analysis Tool (2 Sensors)")
        ...
        self.stats_df = pd.DataFrame(columns=[
            "range", "n_points",
            "B_minus_A_mean",
            "mean_depth_both_sensors",
            "Sensor_A_mean", "Sensor_B_mean",
            "Sensor_A", "Sensor_B", "source_file"
        ])
    ...
    def setup_left_panel(self, parent):
        ...
        cols = ("range", "n_points", "B_minus_A_mean", "mean_depth_both_sensors",
                "Sensor_A_mean", "Sensor_B_mean",
                "Sensor_A", "Sensor_B", "source_file")
    ...
    def process_data(self):
        ...
        try:
            ...
            if not all([sensor_a_col, sensor_b_col]):
                raise ValueError("Could not find both sensor columns")
            
            self.log(f"Found sensors A and B")
            ...
            depth_cols = ['Sensor_A_Depth', 'Sensor_B_Depth']
            ...
            # Create a combined depth column for filtering (use first available sensor)
            df_depth['filter_depth'] = df_depth['Sensor_A_Depth'].fillna(df_depth['Sensor_B_Depth'])
    ...
    def show_statistics(self):
        ...
        # compute mean depth across both sensors for the selection (mean of per-row means)
        mean_depth_both = df_calc[['Sensor_A_Depth', 'Sensor_B_Depth']].mean(axis=1).mean()
        self.log(f"\nMean depth (both sensors): {mean_depth_both:.6f}")
        ...
        self.last_stats = {
            ...
            "mean_depth_both_sensors": float(mean_depth_both),
    ...
    def show_offset_predictor(self):
        ...
        pred_window.title("Offset Predictor")
        ...
        # Perform regression
        depth_data = self.stats_df['mean_depth_both_sensors'].values
        ba_data = self.stats_df['B_minus_A_mean'].values
        
        # Remove NaN values
        mask = ~(np.isnan(depth_data) | np.isnan(ba_data))
        ...
        result_text = scrolledtext.ScrolledText(ctrl_frame, height=8, width=35, wrap=tk.WORD)
        ...
        def predict():
            ...
            result_text.insert(tk.END, f"Linear Model:\n")
            result_text.insert(tk.END, f"y = {res_ba.slope:.6f}x + {res_ba.intercept:.6f}\n\n")
            result_text.insert(tk.END, f"R² = {r_squared:.4f}\n")
        
        ttk.Button(ctrl_frame, text="Calculate", command=predict).pack(fill=tk.X, pady=5)
        
        # Model info frame
        info_frame = ttk.LabelFrame(left_panel, text="Regression Model", padding="10")
        info_frame.pack(fill=tk.X, pady=5)
        
        info_text = scrolledtext.ScrolledText(info_frame, height=10, width=35, wrap=tk.WORD)
        info_text.pack(fill=tk.BOTH, expand=True)
        
        info_text.insert(tk.END, "B - A vs Depth\n")
        info_text.insert(tk.END, "="*35 + "\n")
        info_text.insert(tk.END, f"Slope: {res_ba.slope:.6f}\n")
        info_text.insert(tk.END, f"Intercept: {res_ba.intercept:.6f}\n")
        info_text.insert(tk.END, f"R-squared: {r_squared:.4f}\n")
        info_text.insert(tk.END, f"P-value: {res_ba.pvalue:.6f}\n")
        info_text.insert(tk.END, f"Std Error: {res_ba.stderr:.6f}\n\n")
        ...
        # Scatter plot
        ax.scatter(depth_clean, ba_clean, color='royalblue', label='Measured Mean Bias', alpha=0.7, s=50)
        
        # Trendline
        trendline = res_ba.slope * depth_clean + res_ba.intercept
        ax.plot(depth_clean, trendline, color='red', linewidth=2, 
                label=f'y = {res_ba.slope:.6f}x + {res_ba.intercept:.4f}')
        
        ax.set_xlabel('Mean Depth (m)', fontsize=11)
        ax.set_ylabel('B - A Bias (m)', fontsize=11)
        ax.set_title(f'B - A vs Depth (R² = {r_squared:.3f})', fontsize=12, fontweight='bold')
        ax.legend(loc='best')
...
if __name__ == "__main__":
    root = tk.Tk()
    app = CTDAnalyzerApp2Sensor(root)
    root.mainloop()